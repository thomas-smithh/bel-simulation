...
def target_function(
    ...
    run_number = get_next_run_number("F:\\Bel_Simulation\\Optimisation Output with Initial Parameter Space Probing GOOD RANGE ONLY")
    ...
    simulation.execute(
        ...
        write_results=True,
        write_path="F:\\Bel_Simulation\\Optimisation Output with Initial Parameter Space Probing GOOD RANGE ONLY"
    )
...
if __name__ == '__main__':
    ...
    search_space = {
        ...
        'beta': hp.uniform('beta', 0, 400),
        'A_eq_star_scaling': hp.uniform('A_eq_star_scaling', 0, 0.4),
        'P_star': hp.uniform('P_star', 0, 100),
    }

    beta = np.random.uniform(28.919282, 367.834720	, 5)
    alpha =  np.random.uniform(0.009067, 0.908844, 5)
    A_eq_star_scaling = np.random.uniform(0.078842, 0.387942, 5)
    P_star = np.random.uniform(0.258949	, 1.884359, 5) 
    ...
    best = fmin(
        ...
        trials=trials,
        verbose=True,
        trials_save_file='BEL_trials_15_Optimising_with_initial_probing_lumenvolcost_fccstart_pos_goodrangeonly.pkl'
    )